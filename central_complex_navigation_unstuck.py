"""
Central Complex Navigation System (Unstuck Exploration)
=====================================================

This version adds stuck detection and wall repulsion to prevent the agent from getting stuck in corners or at boundaries. If the agent hasn't moved significantly for several steps, it will randomly reorient. Wall repulsion discourages sticking to boundaries. All other logic is as in the exploration-enhanced version.
...
# -----------------------------
# Central Complex Navigator (with stuck detection and wall repulsion)
# -----------------------------

class CentralComplexNavigator:
    def __init__(self, env_size=100, dt=0.2, cpu4_gain=1.0, cpu4_decay=0.0, cpu1_interference=0.0, coverage_resolution=2, stuck_steps=20, stuck_threshold=1.0, wall_repulsion_dist=8, initial_position=None, initial_heading=None, initial_state='exploration'):
        ...
        if initial_heading is not None:
            self.heading = initial_heading % (2*np.pi)
        else:
        ...
        self.recent_positions = [self.position.copy()]
        self.is_stuck = False
        # Wall repulsion
        self.wall_repulsion_dist = wall_repulsion_dist
    ...
    def _check_stuck(self):
        self.recent_positions.append(self.position.copy())
        if len(self.recent_positions) > self.stuck_steps:
            self.recent_positions.pop(0)
        dists = [np.linalg.norm(self.position - p) for p in self.recent_positions]
        if max(dists) < self.stuck_threshold:
            self.is_stuck = True
        else:
            self.is_stuck = False
    ...
    def _wall_repulsion(self):
        repulse = 0.0
        margin = self.wall_repulsion_dist
        if self.position[0] < margin:
            repulse += (margin - self.position[0]) / margin
        if self.position[0] > self.env_size - margin:
            repulse -= (self.position[0] - (self.env_size - margin)) / margin
        if self.position[1] < margin:
            repulse += (margin - self.position[1]) / margin
        if self.position[1] > self.env_size - margin:
            repulse -= (self.position[1] - (self.env_size - margin)) / margin
        return repulse

    def generate_steering_command(self):
        self._check_stuck()
        wall_repulse = self._wall_repulsion()
        if self.state == 'exploration':
            if self.is_stuck:
                # Random reorientation
                turn = np.random.uniform(-np.pi, np.pi)
                return np.clip(turn, -0.5, 0.5)
            # Bias toward less-visited areas
            look_angles = np.linspace(-np.pi/2, np.pi/2, 9)
            scores = []
            for da in look_angles:
                ...
                if 0 <= x_idx < self.coverage_shape[0] and 0 <= y_idx < self.coverage_shape[1]:
                    score = -self.coverage_map[x_idx, y_idx]
                else:
                    score = -1000
                # Add wall repulsion to score
                score += wall_repulse * 5
                scores.append(score)
    ...
    def run_simulation(self, steps=900):
    ...
    def set_state(self, new_state):
        """Set the agent's behavioral state at runtime."""
...
if __name__ == '__main__':
    # Example: Start at (10, 10), facing right (0 radians), in 'exploration' state
    navigator = CentralComplexNavigator(
        cpu4_gain=1.0,
        cpu4_decay=0.01,
        cpu1_interference=0.02,
        coverage_resolution=2,
        initial_position=[10, 10],
        initial_heading=0.0,
        initial_state='exploration'
    )
    # Change state at runtime (e.g., after 100 steps)
    def change_state_later(navigator, step_trigger=100, new_state='homing'):
        orig_step = navigator.step
        def wrapped_step():
            if len(navigator.trajectory) == step_trigger:
                navigator.set_state(new_state)
            return orig_step()
        navigator.step = wrapped_step
    change_state_later(navigator, step_trigger=100, new_state='homing')
...
"""
Instructions:
- Run this script to launch the unstuck exploration simulation and visualization.
- The agent will systematically explore the environment and avoid getting stuck.
