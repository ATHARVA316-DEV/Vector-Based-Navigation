...
A modular, extensible, and biologically detailed simulation of insect vector-based navigation, based on Le Moël et al. (2019).
Enhancements: clearer class structure, improved visualization, hooks for advanced features, and richer documentation.
...
class TB1Compass:
    ...
    def update(self, heading_rad):
        """Update TB1 activity based on current heading (radians)."""
...
class CPU4Integrator:
    ...
    def integrate(self, delta_x, delta_y):
        """Integrate movement (delta_x, delta_y) into the home vector."""
    ...
    def get_vector(self):
        """Decode the home vector from distributed activity."""
...
class CPU1Memory:
    ...
    def store(self, cpu4_activity, reward_strength=1.0):
        ...
        # Add interference/noise to simulate memory overlap
    ...
    def consolidate(self, memory_id):
        """Strengthen a memory (plasticity demonstration)."""
        if 0 <= memory_id < len(self.memories):
...
class CentralComplexNavigator:
    ...
    """
    def __init__(self, env_size=100, dt=0.2, cpu4_gain=1.0, cpu4_decay=0.0, cpu1_interference=0.0):
    ...
    def update_neural_activity(self, movement, sensory_input=None):
        """Update all neural circuits based on movement and sensory input."""
    ...
    def generate_steering_command(self):
        """Generate a steering command based on current behavioral state."""
        ...
        if self.state == 'homing':
            ...
            # Shortcut: vector from food A to food B
            ...
            # Route optimization: nearest food
    ...
    def _update_performance_metrics(self):
        # Example: track distance to nest and food
    ...
    def run_simulation(self, steps=600):
...
if __name__ == '__main__':
    navigator = CentralComplexNavigator(cpu4_gain=1.0, cpu4_decay=0.01, cpu1_interference=0.02)
    navigator.run_simulation(steps=700)
...
"""
Instructions:
- Run this script to launch the improved simulation and visualization.
...
- Extend with additional features (plasticity, error correction, environmental noise) as described in complex.md for advanced research.
""" 